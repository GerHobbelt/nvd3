d3 = function() {
  ...
  function d3_geo_albersUsaInvert(projection, extent) {
    var a = projection(extent[0]), b = projection([ 0.5 * (extent[0][0] + extent[1][0]), extent[0][1] ]), c = projection([ extent[1][0], extent[0][1] ]), d = projection(extent[1]);
    var dya = b[1] - a[1], dxa = b[0] - a[0], dyb = c[1] - b[1], dxb = c[0] - b[0];
    var ma = dya / dxa, mb = dyb / dxb;
    var cx = 0.5 * (ma * mb * (a[1] - c[1]) + mb * (a[0] + b[0]) - ma * (b[0] + c[0])) / (mb - ma), cy = (0.5 * (a[0] + b[0]) - cx) / ma + 0.5 * (a[1] + b[1]);
  ...
  function d3_scale_log(linear, base, log, pow) {
    ...
    scale.tickFormat = function(n, format) {
      ...
      var b = Math.log(base), k = Math.max(0.1, n / scale.ticks().length), f = log === d3_scale_logn ? (e = -1e-12, 
